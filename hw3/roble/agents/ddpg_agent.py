...
from hw3.roble.infrastructure.utils import OUNoise
import copy
...
class DDPGAgent(object):
    ...
    def __init__(self, env, **kwargs):
        ...
        self.noise_gen = OUNoise(self._num_actions)
    ...
    def step_env(self):
        ...
        perform_random_action = np.random.randn(self._num_actions)
        # perform_random_action = self.noise_gen.sample()
        # HINT: take random action
        enc_obs = self._replay_buffer.encode_recent_observation()
        action = self._actor.get_action(enc_obs)
        action = np.clip(action + perform_random_action,
